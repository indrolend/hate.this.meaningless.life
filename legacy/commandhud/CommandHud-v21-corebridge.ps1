...
function Complete-Command([bool]$cancelled = $false) {
    ...
    if (-not $cancelled -and $script:HudCli -and (Test-Path -LiteralPath $script:HudCli)) {
        try {
            ...
            if ($LASTEXITCODE -eq 0 -and $hudJson) {
                $hudRecord = $hudJson | ConvertFrom-Json
                $display = [System.Collections.Generic.List[string]]::new()
                $view = $hudRecord.presentation

                $displayStatus = ([string]$view.status).ToUpperInvariant()
                $seconds = [math]::Round(([double]$view.durationMs / 1000.0), 1)
                $display.Add(('{0} - {1}s' -f $displayStatus, $seconds))

                if ($view.request) {
                    $display.Add('')
                    $display.Add([string]$view.request)
                }

                if ($view.headline) {
                    $display.Add('')
                    $display.Add([string]$view.headline)
                }

                foreach ($line in @($view.details)) {
                    if ($line) { $display.Add([string]$line) }
                }

                $output = ($display -join "`r`n")
                $exitCode = [int]$hudRecord.exitCode
                $status = if ($exitCode -eq 0) { 'passed' } else { 'failed' }
            }
        } catch {
            # Fall back to the complete transport output already captured above.
        }
    }
...
function Start-Command {
    ...
    if ($command -match '^@hud\s+(\S+)\s+"([^"]+)"\s+(\S+)\s+(\d+)\s+(\d+)\s+::\s*(.+)$') {
        $workflowId = $matches[1]
        $workflowName = $matches[2]
        $workflowStage = $matches[3]
        $workflowIndex = [int]$matches[4]
        $workflowCount = [int]$matches[5]
        $command = $matches[6]
