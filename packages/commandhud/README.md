...
`hud proof <repository-command>` performs a read-only retained-evidence lookup. The terminal-native form is `/proof <repository-command>`; unlike typing `hud proof` as an ordinary nested shell command, the slash form does not create an outer terminal-command record. Both validate the current command identity, scan immutable run metadata, and report whether a successful proof applies to the exact current repository bytes. A commit-only HEAD change remains visible but does not invalidate byte-identical proof. A newer failed attempt against the same bytes fails closed. Proof lookup never executes the command, creates a run, or automatically skips later work.

`hud impact <repository-command>` is the narrower incremental-evidence experiment. It selects the newest complete successful record, derives factual changed paths from that run's retained Git worktree tree, and recovers stage result markers without executing the command or creating a run. A repository may attach a `stageMarker` and literal repository file/directory `stages[].paths` to its existing command declaration. The stage model is copied into the immutable operation record when execution occurs; impact never applies a newer dependency declaration retroactively. Exact whole-repository bytes make recorded passing stages `CURRENT`; after a delta, only an unchanged explicitly declared scope can remain `CURRENT`. Missing scopes, changed declarations, unavailable trees, missing markers, and incomplete raw evidence fail closed to `UNKNOWN`, `STALE`, or `MISSING`. This command reports reusable evidence but never skips or re-derives verification.

`hud storage [--json]` inventories the complete configured evidence store without creating, changing, or deleting runs. It reports physical byte/file/run/project totals, the oldest and newest evidence, integrity classifications, reversible runs, benchmark-scale raw output, and the largest run directories. Integrity checking uses the same retained SHA-256 evidence contract as proof and impact, so an exact inventory may read large evidence files. Deletion and retention policy remain explicit future work; this command is observation only.
...
Handoff generation reads bounded tails directly from evidence files; it does not load an enormous raw log and truncate it afterward. Explicit `/raw` and other retained-evidence commands remain the path for requesting more of the immutable record.

Retained `/head`, `/tail`, `/find`, and `/around` projections scan evidence incrementally and retain at most bounded line representations. Their total-line and match counts remain factual even when a raw record contains a single enormous line. `/raw` is intentionally the explicit unbounded projection.

Run-to-run `/diff` streams ordinary textual patches within explicit line and character limits. For evidence files larger than 8 MiB it compares complete streaming SHA-256 identities and returns a compact factual size/hash difference instead of materializing a potentially enormous text patch.
...
Ordinary execution writes complete child stdout/stderr bytes directly to immutable evidence files while retaining only a bounded 1 MiB tail per stream in memory. File writes apply stream backpressure, and presentation additionally bounds individual lines. Strict opted-in result markers are collected incrementally, so markers before or after enormous output survive without retaining the intervening output in RAM. Search and Lint also parse their structured facts incrementally: complete raw output remains on disk while retained match lines and diagnostic objects are explicitly bounded.

Each newly finalized run records the exact byte length and streaming SHA-256 identity of both raw evidence streams. Proof and impact lookup reject missing or hash-mismatched evidence. Older records without captured hashes remain `UNKNOWN` integrity for backward compatibility; they are not mislabeled as cryptographically verified.
